...
export function getDadosChamados48h(chamados, agora) {
  const inicio = new Date(agora.getTime() - 48 * 3_600_000)
  return Array.from({ length: 13 }, (_, i) => {
    const horaLabel = i * 4
    if (i === 0) return { hora: '0h', chamados: 0 }
    const ini = new Date(inicio.getTime() + (horaLabel - 4) * 3_600_000)
    const fim = new Date(inicio.getTime() + horaLabel * 3_600_000)
